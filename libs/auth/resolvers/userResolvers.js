    ...
    Mutation: {
        ...
        register: async (parent, args, context, info) => {
            ...
            try {
                ...
                if (!email.toLowerCase().includes('test.com')) {
                    await context.dataSources.sendgridAPI.newContact(email.toLowerCase())
                    await context.dataSources.jarvisAPI.sendMessage(`New Lendbox User: ${email.toLowerCase()}`)
                }
        ...
        login: async (parent, args, context, info) => {
            ...
            const handlePermissionUserLogin = async () => {
                const permissionUser = await PermissionUserModel.findOne({ email: args.user.email.toLowerCase() })

                const { email, password } = permissionUser
                const isMatch = await comparePassword(args.user.password, password)
                if (isMatch) {
                    const token = getToken({ email: email.toLowerCase(), password })
                    return { email, password, token, isLoggedIn: true };
            ...
            if (user) {
                ...
                const permissionUser = await PermissionUserModel.findOne({ email })

                if (permissionUser) {
                    return await handlePermissionUserLogin()
                } else {
                    throw new AuthenticationError(constants.WRONG_PASSWORD)
                }
            }
